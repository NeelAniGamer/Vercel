...
const GamePage = () => {
  ...
  useEffect(() => {
    const initGameSession = async () => {
      try {
        ...
        const { data: def } = await supabase.from('achievement_definitions').select('id').eq('slug', 'global_score').single()
  ...
  return (
    <div className="relative h-screen w-screen">
      ...
      <AchievementToast isVisible={!!achievement} title={achievement?.title || ''} message={achievement?.message || ''} onClose={closeAchievement} />
    </div>
  )
}
