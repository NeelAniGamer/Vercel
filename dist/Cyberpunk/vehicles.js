...
window.IndianVehicles = {
  ...
  buildVehicle: function (type, colorHex) {
    ...
    if (type === 'splendor' || type === 'bike' || type === 'twowheeler') {
      ...
      body.position.y = 0.6
      const w1 = this.buildWheel()
      w1.position.set(0, 0.35, -0.7)
      const w2 = this.buildWheel()
    ...
    // --- THREE WHEELERS ---
      ...
      const w1 = this.buildWheel()
      w1.position.set(0, 0.35, -0.9)
      const w2 = this.buildWheel()
      w2.position.set(-0.6, 0.35, 0.8)
      const w3 = this.buildWheel()
      w3.position.set(0.6, 0.35, 0.8)
