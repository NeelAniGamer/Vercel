_buildScene(mode) {
        ...
        const mats = {
          grass: new THREE.MeshLambertMaterial({ color: cfg.ground || 0x33691e }),
          road: new THREE.MeshLambertMaterial({ color: 0x21232b, map: _genTex('asphalt'), roughness: 0.9, metalness: 0.1 }),
          pave: new THREE.MeshLambertMaterial({ color: 0x757575, map: _genTex('pave') }),
          yellowLine: new THREE.MeshBasicMaterial({ color: 0xffcc00 }),
          water: new THREE.MeshLambertMaterial({ color: 0x1a5a8a, transparent: true, opacity: 0.7 })
        };
        this.roadMat = mats.road;
        ...
        if (window.PRELOADED_MODELS) {
            Object.keys(instancedBldgData).forEach(key => {
                ...
                meshes.forEach(mesh => {
                    ...
                    instancedMesh.castShadow = false; // Disabled for performance
        ...
        (cfg.ints || []).forEach(([ix, iz]) => {
          this._sig(ix + 7.5, iz + 7.5);

          if (window.PRELOADED_MODELS && window.PRELOADED_MODELS['road_cross']) {
             const intTile = window.PRELOADED_MODELS['road_intersect'] ? window.PRELOADED_MODELS['road_intersect'].clone() : window.PRELOADED_MODELS['road_cross'].clone();
             const tileScale = RW;
        ...
        if (cfg.hasRain) {
          ...
          for (let i = 0; i < 15; i++) {
            const p = new THREE.Mesh(new THREE.CylinderGeometry(1.4 + Math.random(), 1.5 + Math.random(), .08, 12), new THREE.MeshLambertMaterial({ color: 0x0c101a, transparent: true, opacity: 0.6 }));
        ...
        if (cfg.hasEmergency || cfg.id === 8) {
          ...
          if (!this.ms.amb.children.some(c => c.type === 'PositionalAudio')) {
              const siren = new THREE.PositionalAudio(this.audioListener);
              siren.setRefDistance(10);
              siren.setMaxDistance(100);
              const ctx = siren.context;
              const osc = ctx.createOscillator();
              osc.type = 'square';
              osc.frequency.value = 600;
              const lfo = ctx.createOscillator();
              lfo.type = 'sine';
              lfo.frequency.value = 1.5; // Fast wail
              ...
              osc.start();
              lfo.start();
              siren.setNodeSource(osc);
              this.ms.amb.add(siren);
          }

          // Removed expensive PointLights for performance
        ...
        if (cfg.id === 1) {
            ...
            const saMat = new THREE.MeshLambertMaterial({ color: 0xe0e0e0 });
            ...
            const stoneMat = new THREE.MeshLambertMaterial({ color: 0xdfd3c3 }); // Basalt color
        ...
        // ── MOUNTAIN BACKDROP ──
        if (cfg.hasMountain) {
          const mM = new THREE.MeshLambertMaterial({ color: 0x4a6040 });
          const rM = new THREE.MeshLambertMaterial({ color: 0x7a6d5c });
          [
          ...
          for (let rz = -350; rz <= 100; rz += 8) {
            const grL = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.7, 3), new THREE.MeshLambertMaterial({ color: 0xcccccc }));
        ...
        if (cfg.hasRailway) {
          (cfg.railZ || []).forEach(rz => {
            ...
            for (let t = -25; t < 25; t += 2) {
              const rail = new THREE.Mesh(new THREE.BoxGeometry(0.1, .05, 1.5), new THREE.MeshLambertMaterial({ color: 0x888888 }));
              rail.position.set(t, .04, rz); this.scene.add(rail);
            }
            // Crossbar ties
            for (let t = -25; t < 25; t += 4) {
              const tie = new THREE.Mesh(new THREE.BoxGeometry(3, .08, .3), new THREE.MeshLambertMaterial({ color: 0x4a3728 }));
              tie.position.set(t, .03, rz); this.scene.add(tie);
            }
            // Gate poles
            [-8, 8].forEach(gx => {
              const pole = new THREE.Mesh(new THREE.CylinderGeometry(.08, .08, 3, 8), new THREE.MeshLambertMaterial({ color: 0xcc0000 }));
              pole.position.set(gx, 1.5, rz + 7); this.scene.add(pole);
              const arm = new THREE.Mesh(new THREE.BoxGeometry(6, .12, .12), new THREE.MeshLambertMaterial({ color: 0xcc0000 }));
        ...
        if (cfg.isBridge) {
          ...
          const tollM = new THREE.MeshLambertMaterial({ color: 0x888888 });
          const tollY = new THREE.MeshLambertMaterial({ color: 0xffcc00 });
          // 3 toll pillars
          [-8, 0, 8].forEach(ox => {
          ...
          const sign = new THREE.Mesh(new THREE.BoxGeometry(12, 2, 0.2), new THREE.MeshLambertMaterial({ color: 0x003399 }));
          sign.position.set(0, 5.5, -120); this.scene.add(sign);
          // Bridge railings
          [-7.5, 7.5].forEach(rx => {
            for (let z = -600; z < 100; z += 8) {
              const post = new THREE.Mesh(new THREE.CylinderGeometry(.06, .06, 1.5, 6), new THREE.MeshLambertMaterial({ color: 0xcccccc }));
              post.position.set(rx, .75, z); this.scene.add(post);
            }
            const cable = new THREE.Mesh(new THREE.BoxGeometry(.04, 700, .04), new THREE.MeshLambertMaterial({ color: 0xdddddd }));
            cable.position.set(rx, 1.5, -250); cable.rotation.x = Math.PI / 2; this.scene.add(cable);
          });
          // Bridge pylons
          [-200, 0, -400].forEach(pz => {
            const pylon = new THREE.Mesh(new THREE.CylinderGeometry(.8, .6, 25, 8), new THREE.MeshLambertMaterial({ color: 0xcccccc }));
        ...
        if (!cfg.isPedestrian && !cfg.isBridge) {
          const buildLandmark = (type, bx, bz) => {
            ...
            if (type === 'gateway') {
              const m1 = new THREE.Mesh(new THREE.BoxGeometry(20, 18, 12), new THREE.MeshLambertMaterial({ color: 0xd4a373 }));
              ...
              const m3 = new THREE.Mesh(new THREE.CylinderGeometry(2, 2, 6, 8), new THREE.MeshLambertMaterial({ color: 0xd4a373 }));
              m3.position.set(-8, 21, 0); lg.add(m3);
              const m4 = m3.clone(); m4.position.set(8, 21, 0); lg.add(m4);
            } else if (type === 'bse') {
              const m1 = new THREE.Mesh(new THREE.CylinderGeometry(8, 8, 45, 16), new THREE.MeshLambertMaterial({ color: 0xcccccc }));
              m1.position.y = 22.5; lg.add(m1);
              const m2 = new THREE.Mesh(new THREE.CylinderGeometry(10, 10, 5, 16), new THREE.MeshLambertMaterial({ color: 0x444444 }));
              m2.position.y = 47.5; lg.add(m2);
            } else if (type === 'antilia') {
              for (let i = 0; i < 8; i++) {
                const w = 12 + Math.random() * 8;
                const m = new THREE.Mesh(new THREE.BoxGeometry(w, 5, w), new THREE.MeshLambertMaterial({ color: (i % 2 === 0) ? 0x88aa88 : 0xaaaaaa }));
        ...
        if (cfg.hasSchool) {
          // School building
          const school = new THREE.Mesh(new THREE.BoxGeometry(18, 8, 12), new THREE.MeshLambertMaterial({ color: 0xd4ac0d }));
          school.position.set(-40, 4, -80); this.scene.add(school); this.obstacles.push(school);
          // School sign
          const sign = new THREE.Mesh(new THREE.BoxGeometry(3, 1.5, .1), new THREE.MeshLambertMaterial({ color: 0xffff00 }));
        ...
        if (cfg.hasBeach) {
          const sand = new THREE.Mesh(new THREE.PlaneGeometry(200, 600), new THREE.MeshLambertMaterial({ color: 0xc2b280 }));
        ...
        if (cfg.hasSilentZone) {
          // Hospital building
          const hosp = new THREE.Mesh(new THREE.BoxGeometry(20, 12, 15), new THREE.MeshLambertMaterial({ color: 0xeeeeee }));
          hosp.position.set(25, 6, -20); this.scene.add(hosp); this.obstacles.push(hosp);
          const cross = new THREE.Mesh(new THREE.BoxGeometry(2, 2, .1), new THREE.MeshLambertMaterial({ color: 0xff0000 }));
          cross.position.set(25, 10, 7.6); this.scene.add(cross);
          // Silent zone markers
          [cfg.silentZ1 || 0, cfg.silentZ2 || 0].forEach(sz => {
            const marker = new THREE.Mesh(new THREE.BoxGeometry(1, 2, .1), new THREE.MeshLambertMaterial({ color: 0xff6600 }));
        ...
        for (let i = 0; i < bCount; i++) {
          ...
          const bp1 = new THREE.Mesh(new THREE.CylinderGeometry(.06, .06, 1.5, 8), new THREE.MeshLambertMaterial({ color: 0xff3300 }));
          bp1.position.set(-0.5, 0.75, 0); barG.add(bp1);
          const bp2 = bp1.clone(); bp2.position.set(0.5, 0.75, 0); barG.add(bp2);
          const bBar = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.25, 0.15), new THREE.MeshLambertMaterial({ color: 0xffffff }));
          bBar.position.set(0, 1.3, 0); barG.add(bBar);
          const rSt = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.25, 0.16), new THREE.MeshLambertMaterial({ color: 0xff0000 }));
