class Game {
      ...
      _horn() { 
          if (this.isPedestrian) return; // Only beep when driving
      ...
      _input(dt) {
        ...
        let maxR = this.isPedestrian ? 4.8 : 6.0;
        this.roadSegments.forEach(r => {
          if (r.type === 'v' && Math.abs(this.player.position.x - r.x) < maxR) validRoadBound = true;
          if (r.type === 'h' && Math.abs(this.player.position.z - r.z) < maxR) validRoadBound = true;
        });
