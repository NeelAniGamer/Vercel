...
// 1. Build the new _buildHuman implementation
const newBuildHuman = `const _buildHuman = (isPlayer = false, appearance) => {
...
// Find _buildHuman start and end
...
if (bhStart === -1 || bhEnd === -1) {
  console.error('Failed to locate _buildHuman boundaries');
  process.exit(1);
}

// Find Customizer start and end using regex
...
if (!custStartMatch || !custEndMatch) {
  console.error('Failed to locate Customizer boundaries with regex');
  process.exit(1);
}
...
// Replace Customizer first
uiContent = uiContent.slice(0, custStart) + newCustomizerLogic + '\n\n  ' + uiContent.slice(custEnd);

// Recalculate bhStart and bhEnd after first replacement
...
uiContent = uiContent.slice(0, bhStart2) + newBuildHuman + '\n\n' + uiContent.slice(bhEnd2);
...
console.log('Successfully upgraded _buildHuman and Customizer Studio in ui.js!');
