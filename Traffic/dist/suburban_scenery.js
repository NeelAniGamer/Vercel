...
(function () {
  ...
  function createSuburbanNeighborhood(game, cfg) {
    ...
    const mCurbRamp = new THREE.MeshLambertMaterial({ color: 0xf1c40f, roughness: 0.8 });
    const texGrassInit = (typeof createGrassCanvasTexture === 'function') ? createGrassCanvasTexture() : null;
    const mGrass = texGrassInit
      ? new THREE.MeshLambertMaterial({ map: texGrassInit, roughness: 0.85 })
      : new THREE.MeshLambertMaterial({ color: 0x3da339, roughness: 0.85 });
    const mHedge = new THREE.MeshLambertMaterial({ color: 0x1b4332, roughness: 0.8 });
