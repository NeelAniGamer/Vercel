// @ts-nocheck
/**
 * GameplayRecorder — migrated from gameplay-recorder.js
 * Records violations for the post-level review
 */
...
export class GameplayRecorder {
  ...
  private _levelId: any = null;
  private _levelName: string = '';

  static categories = CATEGORIES;

  start(levelId: any, levelName?: string): void {
    this._events = [];
  ...
  record(type: string, data: any = {}): void {
    const meta = EVENT_META[type] || { label: type, icon: '📌', category: 'Other', severity: 'info', tip: '' };
  ...
  getEvents(): RecordedEvent[] { return this._events; }

  getSummary(): any {
    const events = this._events;
    const byCategory: Record<string, RecordedEvent[]> = {};
    const bySeverity: Record<string, number> = { info: 0, warning: 0, danger: 0, critical: 0 };
    const mistakes: RecordedEvent[] = [];
  ...
  showReview(onContinue?: () => void): void {
    ...
    const gradeColors: Record<string, string> = { 'A+': '#22c55e', 'A': '#22c55e', 'B+': '#06b6d4', 'B': '#3b82f6', 'C+': '#f59e0b', 'C': '#f97316', 'D': '#ef4444' };
    ...
    const timelineHtml = timeline.length > 0 ? `
      <div class="dr-timeline-wrap">
        <div class="dr-timeline-track">
          ${timeline.map(ev => `<div class="dr-timeline-dot" style="background:${ev.color};" title="${ev.label} (${ev.timeFormatted})"></div>`).join('')}
        </div>
        ...
        <div class="dr-mistake-list">
          ${mistakes.map(ev => `
            <div class="dr-mistake-row" style="border-left:3px solid ${ev.severity === 'critical' ? '#ef4444' : ev.severity === 'danger' ? '#f97316' : '#f5900b'};">
              ...
              <span class="dr-mistake-time">${this._formatTime(ev.time)}</span>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    const catBreakdownHtml = catEntries.length > 0 ? `
      <div class="dr-section">
        ...
        <div class="dr-cat-grid">
          ${catEntries.map(([cat, events]: [string, any[]]) => {
            const catMeta = CATEGORIES[cat] || { icon: '📌', color: '#64748b' };
    ...
    if (!document.getElementById('dr-styles')) {
      ...
      style.id = 'dr-styles';
      style.textContent = `
        .dr-overlay { position: fixed; inset: 0; z-index: 10001; display: flex; align-items: center; justify-content: center; padding: 20px; opacity: 0; transition: opacity 0.3s ease; }
        .dr-overlay.show { opacity: 1; }
        .dr-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.75); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
        .dr-card { position: relative; z-index: 1; background: #0f172a; border: 1px solid rgba(255,255,255,0.12); border-radius: 20px; width: min(520px, 95vw); max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 24px 64px rgba(0,0,0,0.5); transform: translateY(20px) scale(0.97); transition: transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); font-family: 'Inter', sans-serif; }
        .dr-overlay.show .dr-card { transform: translateY(0) scale(1); }
        .dr-header { padding: 24px 24px 16px; border-bottom: 1px solid rgba(255,255,255,0.08); flex-shrink: 0; }
        ...
        .dr-close { width: 32px; height: 32px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: #94a3b8; font-size: 1.2rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
        .dr-close:hover { background: rgba(255,255,255,0.1); color: #fff; }
        .dr-body { padding: 20px 24px; overflow-y: auto; flex: 1; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.15) transparent; }
        .dr-footer { padding: 16px 24px; border-top: 1px solid rgba(255,255,255,0.08); flex-shrink: 0; }
        .dr-btn-primary { width: 100%; padding: 14px; border: none; border-radius: 12px; background: linear-gradient(135deg, #f59e0b, #d97706); color: #000; font-weight: 800; font-size: 1rem; cursor: pointer; transition: all 0.2s; font-family: inherit; }
        ...
        .dr-stat { flex: 1; text-align: center; padding: 14px 8px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; }
        ...
        .dr-timeline-track { display: flex; gap: 4px; align-items: center; padding: 12px 8px; background: rgba(255,255,255,0.03); border-radius: 10px; border: 1px solid rgba(255,255,255,0.06); overflow-x: auto; scrollbar-width: none; }
        .dr-timeline-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; transition: transform 0.2s; cursor: pointer; }
        .dr-timeline-dot:hover { transform: scale(1.6); }
        .dr-timeline-labels { display: flex; justify-content: space-between; padding: 4px 8px 0; font-size: 0.6rem; color: #64748b; }
        .dr-section { margin-bottom: 20px; }
        .dr-section-title { font-size: 0.75rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 10px; }
        .dr-mistake-list { display: flex; flex-direction: column; gap: 6px; }
        .dr-mistake-row { display: flex; align-items: center; gap: 10px; padding: 10px 12px; background: rgba(255,255,255,0.03); border-radius: 8px; font-size: 0.85rem; }
        ...
        .dr-tip { display: flex; gap: 10px; padding: 10px 12px; background: rgba(34,197,94,0.06); border: 1px solid rgba(34,197,94,0.15); border-radius: 8px; font-size: 0.8rem; color: #cbd5e1; line-height: 1.5; }
        ...
        .dr-cat-item { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: rgba(255,255,255,0.03); border-radius: 8px; font-size: 0.8rem; }
...
// Singleton instance (matches legacy window.GameplayRecorder API)
export const gameplayRecorder = new GameplayRecorder();

// Legacy global access
if (typeof window !== 'undefined') {
  (window as any).GameplayRecorder = gameplayRecorder;
}