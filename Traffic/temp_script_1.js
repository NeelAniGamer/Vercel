    ...
    window.addEventListener('col-auth-changed', (e) => {
        ...
        const cloudLoginBtns = document.querySelectorAll('.cloud-login-btn');
        cloudLoginBtns.forEach(btn => {
            if (customUser) {
                btn.innerHTML = '☁️ ' + (customUser.name ? customUser.name.split(' ')[0] : 'Saved');
                btn.style.background = '#4CAF50';
                btn.style.borderColor = '#4CAF50';
            } else {
                btn.innerHTML = '☁️ Cloud Login';
                btn.style.background = '#5ED4F5';
                btn.style.borderColor = '#5ED4F5';
            }
        });
    ...
    function injectConflictModal(cloudS) {
        ...
        <div class="md" style="background:#111827; color:#E8E3D8; padding:20px; border-radius:12px; max-width:400px; margin:auto; margin-top:100px; z-index: 10001; position: relative;">
            <div class="md-hd" style="border-bottom: 1px solid rgba(255,255,255,0.08); margin-bottom: 15px; padding-bottom: 15px;">
                <h2 style="font-family:'Instrument Serif', serif; font-size:2rem; margin-bottom:5px;">☁️ Cloud Sync Conflict</h2>
            ...
            <div class="md-body" style="display:flex; flex-direction:column; gap:10px;">
                <button class="btn" onclick="resolveConflict('cloud')" style="background:#5ED4F5; color:#000; padding:12px; font-weight:bold; border-radius:6px; border:none; cursor:pointer;">⬇️ Download Cloud Data</button>
                <button class="btn" onclick="resolveConflict('local')" style="background:#F2B84B; color:#000; padding:12px; font-weight:bold; border-radius:6px; border:none; cursor:pointer;">⬆️ Keep Local Data (Overwrite Cloud)</button>
            </div>
        </div>`;
...
switch (type) {
  ...
  default: {
    ...
    class Game {
      ...
      _initR() {
        const cv = document.getElementById('3c'); this.renderer = new THREE.WebGLRenderer({ canvas: cv, antialias: true, powerPreference: "high-performance" });
