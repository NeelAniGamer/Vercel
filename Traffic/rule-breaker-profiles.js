...
const RULE_BREAKER_TYPES = {
  signal_jumper: {
    ...
    behaviors: {
      signalCompliance: 0.15,
      patience: 0.2,
      aggression: 0.7,
      laneDiscipline: 0.6,
      speedVariance: 0.3,
      overtakeThreshold: 0.25,
      sidewalkProbability: 0.05,
      parkingSkill: 0.4
    },
  ...
  sidewalk_rider: {
    ...
    behaviors: {
      ...
      laneDiscipline: 0.1,
      speedVariance: 0.45,
      overtakeThreshold: 0.1,
      sidewalkProbability: 0.85,
      parkingSkill: 0.2
    },
  ...
  horn_abuser: {
    ...
    behaviors: {
      signalCompliance: 0.7,
      patience: 0.1,
      aggression: 0.5,
      laneDiscipline: 0.7,
      speedVariance: 0.2,
      overtakeThreshold: 0.4,
      sidewalkProbability: 0.0,
      parkingSkill: 0.6
    },
...
const MUMBAI_TRAFFIC_STATS_2024_2026 = {
  signalJump: {
    year: 2024,
    count: 12847,
    fine: 1000,
    points: 5,
    source: 'Mumbai Traffic Police Annual Report 2024',
    trend: '+18% from 2023'
  },
  sidewalkBikes: {
    year: 2025,
    percentage: 45,
    description: 'Pedestrian injuries from bikes on footpaths',
    source: 'MTP Pedestrian Safety Report 2025',
    trend: 'Stable'
  },
  wrongSide: {
    year: 2024,
    ...
    fine: 1500,
    points: 6,
    source: 'MTP Enforcement Data 2024',
    trend: '+23% from 2023'
  },
  laneCutting: {
    year: 2025,
    percentage: 31,
    description: 'Non-fatal crashes caused by lane cutting on arterial roads',
    source: 'MTP Accident Analysis 2025',
    trend: '+5% from 2024'
  },
  noisePollution: {
    year: 2024,
    avgDb: 78,
    whoLimit: 55,
    source: 'CPCB Noise Monitoring 2024',
    trend: 'Exceeds limit by 42%'
  },
  towing: {
    year: 2024,
    count: 8932,
    avgFine: 2500,
    source: 'MTP Towing Records 2024',
    trend: '+12% from 2023'
  },
  zebraBlocking: {
    year: 2025,
    percentage: 28,
    description: 'Pedestrian near-misses at blocked zebra crossings',
    source: 'MTP Pedestrian Safety Audit 2025',
    trend: 'New metric'
  },
  highBeam: {
    year: 2025,
    percentage: 12,
    description: 'Night-time reaction delays from high beam glare',
    source: 'IIT Bombay Night Driving Study 2025',
    trend: 'New metric'
  },
  helmetCompliance: {
    year: 2024,
    percentage: 67,
    description: 'Two-wheeler rider helmet compliance rate',
    source: 'MTP Helmet Enforcement 2024',
    trend: '+8% from 2023'
  },
  seatbeltCompliance: {
    year: 2024,
    percentage: 72,
    description: 'Four-wheeler seatbelt compliance (rear: 31%)',
    source: 'MTP Seatbelt Survey 2024',
    trend: 'Rear compliance critical gap'
  },
  drunkDriving: {
    year: 2024,
    cases: 2847,
    fatalities: 89,
    source: 'MTP Drunk Driving Data 2024',
    trend: '-15% from 2023 (stricter enforcement)'
  },
  pedestrianFatalities: {
    year: 2024,
    count: 412,
    percentage: 52,
    description: 'Pedestrians as % of total road fatalities',
    source: 'MTP Fatality Report 2024',
    trend: 'Highest in metro cities'
  }
};

function getMumbaiStat(key) {
  return MUMBAI_TRAFFIC_STATS_2024_2026[key] || null;
}

function getAllMumbaiStats() {
  return MUMBAI_TRAFFIC_STATS_2024_2026;
}

function getConsequenceModalData(violationType) {
  const statMap = {
    'signal_jump': 'signalJump',
    'sidewalk_ride': 'sidewalkBikes',
    'wrong_side': 'wrongSide',
    'lane_cut': 'laneCutting',
    'honking': 'noisePollution',
    'illegal_park': 'towing',
    'zebra_block': 'zebraBlocking',
    'high_beam': 'highBeam'
  };
...
window.RULE_BREAKER_TYPES = RULE_BREAKER_TYPES;
window.RULE_BREAKER_SPAWN_RATE = RULE_BREAKER_SPAWN_RATE;
window.MAX_RULE_BREAKERS_PER_EDGE = MAX_RULE_BREAKERS_PER_EDGE;
window.RuleBreakerProfile = RuleBreakerProfile;
window.pickRuleBreakerType = pickRuleBreakerType;
window.shouldSpawnRuleBreaker = shouldSpawnRuleBreaker;
window.MUMBAI_TRAFFIC_STATS_2024_2026 = MUMBAI_TRAFFIC_STATS_2024_2026;
window.getMumbaiStat = getMumbaiStat;
window.getAllMumbaiStats = getAllMumbaiStats;
window.getConsequenceModalData = getConsequenceModalData;